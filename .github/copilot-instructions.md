...
- ❌ **絕對禁止**：收到實作任務後直接寫程式碼，跳過「實作前 Checklist」（階段 A + B）
- ❌ **絕對禁止**：未完成「需求確認 → 方案確認」就開始修改程式碼（除非使用者明確說「直接改」）
...
> 💡 **快速模式**：當使用者明確說「直接改」或指令已包含完整細節，可合併階段 B 為一步：直接提出方案並等待確認，或跳過確認直接執行。
